...
const retiredProductDestinations = {
  ...
  'turmeric-honey-goat-milk-soap': '/shop',
  'shea-butter-turmeric-gulab': '/shop',
  'honey-kesar-haldi-shea-butter-soap': '/shop',
}

// Combination pages written about bars SoapLedger has since archived. Both were
// indexed and in the sitemap, so they go to their closest live topical parent
// rather than 404ing. The entries stay in src/data/combinations.ts with
// publishedAt: null, so restoring a bar is a one-line change plus dropping the
// redirect below.
const retiredCombinationRedirects = [
  { source: '/honey-shea-butter-soap', destination: '/shea-butter-soap', permanent: true },
  { source: '/rose-goat-milk-soap', destination: '/goat-milk-soap', permanent: true },
]
...
{ source: '/wp-includes/:path*',                   destination: '/blog',     permanent: true },
      ...
      // kesar-haldi-goat-milk-soap is now in the alias map, so its redirect is
      // generated by legacyProductRedirects above rather than listed here.
      { source: '/shop/rice-rose-goat-milk-soap',        destination: '/shop', permanent: true },
      { source: '/shop/turmeric-honey-goat-milk-soap',   destination: '/shop', permanent: true },
      { source: '/shop/shea-butter-turmeric-gulab',      destination: '/shop', permanent: true },
      { source: '/shop/honey-kesar-haldi-shea-butter-soap', destination: '/shop', permanent: true },
